...
# Assuming these imports are correctly placed at the top of your file
from recsys.recall.two_towers.eval import TwoTowerEvaluator
from recsys.recall.ann.ann import AnnoyRecall # Corrected import path
from recsys.recall.two_towers.utils import select_device
# from recsys.data.load_data import load_data # Not needed if loading directly
# from recsys.data.utils import find_products_by_id # Not needed if using map
from recsys.data.embeding import convert_query


# Define constants (consider moving these to a config file or passing as args)
...
# Use absolute path for the model as provided in context
MODEL_PATH = "/Users/haozhezhang/Documents/Python Project/two_tower_search/models/two_towers/final_model.pt"
# Dimensions should match your model and data
INPUT_DIM_Q = 32
INPUT_DIM_P = 160
HIDDEN_Q = 64
HIDDEN_P = 64
EMBED = 32
ANN_QUERY_DIM = 32 # Should match INPUT_DIM_Q or the output of convert_query
ANN_PRODUCT_DIM = 160 # Should match INPUT_DIM_P


# Updated function signature, removing 'm' as it wasn't used in the revised plan
def main(querys: List[str], k: int) -> pd.DataFrame:
    ...
    try:
        ...
        # Get top k // 2 product indices for each query
        num_products_tt = k // 2
    ...
    try:
        ...
        ann_recall = AnnoyRecall(
            query_tower_input_dim=ANN_QUERY_DIM, # Required by init, though query index isn't used here
            product_tower_input_dim=ANN_PRODUCT_DIM
            # Assuming default tree paths are correct
        )
        # Load only the product index, ensure it's loaded before proceeding
        ann_recall._ensure_product_index_loaded() # Will load if needed or raise error
        ...
        k_safe_ann = min(k, num_items_in_index) # How many neighbors to find per seed product
        if k_safe_ann < k:
             print(f"Warning: Requested top {k} neighbors from ANN per seed, but only {num_items_in_index} items in index. Using {k_safe_ann}.")
        ...
        if k_safe_ann > 0:
         ...
         # This might catch issues if the AnnoyRecall class itself depends on files at init time or load fails
         print(f"Error related to Annoy index files: {e}. Skipping ANN recall.")
         # Ensure all_ann_pids remains lists of empty lists if ANN fails entirely
    ...
    for i, query in enumerate(querys):
        ...
        combined_pids = list(tt_pids.union(ann_pids))

        # Optional: Re-rank based on scores/distances if needed.
        # Here, we just take the first k unique combined pids.
        # A more sophisticated approach might involve sorting by original scores/distances.
        final_pids = combined_pids[:k]
        ...
        for pid in final_pids:
            ...
            final_results.append({
                'query': query,
                'pid': pid_int,
                'product_title': mp_product_dict.get(pid_int, "Title Not Found") # Use mapping
            })
